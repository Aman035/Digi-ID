...
contract Identity {
    ...
    struct issuer {
        ...
        string Desc;
        uint256 Status; //0->unverified , 1->pending , 2->Accepted
        string IssueId;
        uint256 ReqCount;
        mapping(uint256 => verifyIdRequest) Request;
    }
    ...
    function requestIssuerAccount(string memory _Desc, string memory _IssueId) public registered(msg.sender) {
        require(
            IssuerDetail[msg.sender].Status == 0, "Either Account is already an Issuer or has a pending issuer request"
        );
    ...
    function verifyIssuerAccount(uint256 _RqNo) public restricted {
        ...
        require(
            IssuerDetail[IssuerAddress].Status == 1,
    ...
    /// @param _RqNo Request No. of Account
    function rejectIssuerAccount(uint256 _RqNo) public restricted {
        require(_RqNo < IssuerVerificationRequest.length, "Request Not Found");
        address IssuerAddress = IssuerVerificationRequest[_RqNo].Owner;
        require(IssuerVerificationRequest[_RqNo].Status == 1, "Request Already Processed");
        require(
    ...
    function deleteId(uint256 _IdNum) public registered(msg.sender) {
        require(_IdNum < UserDetail[msg.sender].IdCount, "Id does not Exist");

        for (uint256 i = _IdNum; i < UserDetail[msg.sender].IdCount - 1; i++) {
    ...
    function AcceptIdRequest(uint256 _ReqNo, string memory _Sign) public registered(msg.sender) issue(msg.sender) {
        ...
        for (uint256 i = 0; i < UserDetail[User].IdCount; i++) {
            if (
                keccak256(bytes(UserDetail[User].Ids[i].Name)) == keccak256(bytes(IssuerDetail[msg.sender].IssueId))
    ...
    /// @param _ReqNo Request No. of Identity
    function RejectIdRequest(uint256 _ReqNo) public registered(msg.sender) issue(msg.sender) {
        require(_ReqNo < IssuerDetail[msg.sender].ReqCount, "Request Not Found");
        require(IssuerDetail[msg.sender].Request[_ReqNo].Status == 1, "Request Already Accepted or Rejected");
    ...
    function getId(uint256 _IdNo, address account) public view returns (identity memory) {
    ...
    function totalIssuer() public view returns (uint256) {
        return Issuer.length;
    }

    /// @return uint Total Identity Verification Requests for an Issuer
    function totalRequest() public view returns (uint256) {
    ...
    function getRequest(uint256 _RqNo) public view issue(msg.sender) returns (verifyIdRequest memory) {
    ...
    function issuerVerificationRequestCount() public view returns (uint256) {
